...
$number_columns = [ $column_1, $column_2, $column_3, $column_4 ];

// Remove columns where no number is set
if ( $column_2['number'] === '' ) { unset( $number_columns[1] ); }
if ( $column_3['number'] === '' ) { unset( $number_columns[2] ); }
if ( $column_4['number'] === '' ) { unset( $number_columns[3] ); }
  ...
  <div class="uk-container uk-container-large uk-flex uk-flex-center uk-flex-wrap uk-child-width-1-2@m uk-child-width-1-3@l uk-child-width-1-4@xl">
    ...
    <?php foreach ( $number_columns as $number_group ) : ?>
      ...
      <div class="number-container">
        <div class="number-ring" data-aos="zoom-out" data-aos-delay="<?php echo esc_attr( $delay ); ?>"></div>

        <?php if ( $number_group['number'] !== '' ) : ?>
          <div class="countup-animated-number">
            <?php if ( $number_group['prepend'] ) : ?>
              <span class="prepend"><?php echo esc_html( $number_group['prepend'] ); ?></span>
            <?php endif; ?>

            <span class="number-span"
              data-delay="<?php echo esc_attr( $delay ); ?>"
              data-start="0"
              data-target="<?php echo esc_attr( $number_group['number'] ); ?>">0</span>

            <?php if ( $number_group['append'] ) : ?>
              <span class="append"><?php echo esc_html( $number_group['append'] ); ?></span>
            <?php endif; ?>
          </div>
        <?php endif; ?>
